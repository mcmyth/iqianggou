...
if( $srcArr["status"]["code"]!=10000 ) break;
    ...
    foreach ($srcArr["data"] as $key => $row)
    {
        ...
        $count++;
           ...
           if($search==null || substr_count($row['name'].$row['distance']. $row['branches'][0]['address']. $row['branches'][0]['redeem_time'],$search)> 0 )
        {
        ...
        $item["data"][$count]["freeze_period"]= $row['freeze_period'];
